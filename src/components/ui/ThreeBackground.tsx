...
export function ThreeBackground() {
  ...
  useEffect(() => {
    ...
    const hubMat = new THREE.MeshBasicMaterial({ color: 0x6366f1, transparent: true, opacity: 0.22, wireframe: true });
    ...
    ringAngles.forEach((tilt, i) => {
      ...
      const rMat = new THREE.MeshBasicMaterial({ color: 0x4f46e5, transparent: true, opacity: 0.1 + i * 0.03 });
    ...
    professorPositions.forEach((pos, i) => {
      ...
      const mat = new THREE.MeshBasicMaterial({ color: 0x818cf8, transparent: true, opacity: 0.18, wireframe: true });
    ...
    for (let i = 0; i < studentCount; i++) {
      ...
      const mat = new THREE.MeshBasicMaterial({ color: 0x6366f1, transparent: true, opacity: 0.45 + Math.random() * 0.25 });
      const mesh = new THREE.Mesh(geo, mat);
    ...
    /* ── Lights ─────────────────────────────────────────────────── */
    scene.add(new THREE.AmbientLight(0x4f46e5, 0.7));  // purple/indigo ambient
