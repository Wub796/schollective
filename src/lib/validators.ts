/**
 * Schollective Validation Engine (server)
 * ─────────────────────────────────────────────────────────────────
 * Zero-dependency, zero-cost algorithms for:
 *   1. Professor application scoring
 *   2. Message content filtering
 *
 * Email validation lives in `email-validation.ts` and is shared with the
 * client-safe entry point; this module only supplies the strict configuration
 * (the full US university domain list) so the two can never drift apart.
 */
...
/**
 * The strict configuration: every curated academic domain plus the full US
 * university list. Used by the professor application scorer below and by the
 * signup enforcement hook in `auth.ts`.
 */
export function validateEmail(
  email: string,
  role: "student" | "professor",
): EmailValidationResult {
  return validateEmailCore(email, role, US_UNIVERSITY_DOMAINS);
}
