        ...
        <div className="w-full max-w-[760px] mx-auto flex flex-col items-center text-center">
          <FadeIn className="w-full flex flex-col items-center">
            ...
            <h1 className="font-display text-[clamp(2.8rem,6.5vw,5rem)] font-black tracking-[-0.04em] leading-[1.05] mb-8 text-slate-900 dark:text-slate-100 text-center">
              Democratizing<br />
              <em className="italic font-light text-indigo-600 dark:text-indigo-400">academic mentorship.</em>
            </h1>
          </FadeIn>
          <FadeIn delay={0.15} className="w-full flex flex-col items-center">
            <p style={{ textAlign: "center" }} className="text-[clamp(1.05rem,1.4vw,1.25rem)] text-slate-600 dark:text-slate-300 leading-relaxed max-w-[620px]">
        ...
        <div className="w-full max-w-[920px] mx-auto">
          <FadeIn>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left">
              <div className="p-8 md:p-10 border border-slate-200/70 dark:border-slate-800 rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shadow-xs flex flex-col gap-4">
                <h2 className="font-display font-bold text-[clamp(1.6rem,2.5vw,2.2rem)] tracking-[-0.03em] leading-snug text-slate-900 dark:text-slate-100">
                  Mentorship is gated by proximity.
                </h2>
                <p className="text-[0.95rem] text-slate-600/90 dark:text-slate-300 leading-relaxed font-sans">
        ...
        <div className="w-full max-w-[920px] mx-auto">
          <FadeIn>
            ...
            <h2 className="font-display text-[clamp(2.2rem,4vw,3.5rem)] font-black tracking-[-0.04em] leading-[1.05] mb-14 text-slate-900 dark:text-slate-100">
              Four principles.<br /><em className="italic font-light text-indigo-600 dark:text-indigo-400">One direction.</em>
            </h2>
          </FadeIn>
            ...
            {VALUES.map((v, i) => (
              <FadeIn key={v.n} delay={i * 0.07}>
                <div className="grid grid-cols-[3.5rem_1fr] gap-8 p-8 md:p-10 mb-6 border border-slate-200/70 dark:border-slate-800 rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md text-left items-start shadow-xs">
                  <span className="font-mono text-[0.68rem] font-bold tracking-[0.2em] text-white bg-indigo-600 px-2.5 py-1 rounded-full text-center mt-1">{v.n}</span>
                  <div>
                    <h3 className="font-display text-[1.35rem] font-bold text-slate-900 dark:text-slate-100 tracking-[-0.02em] mb-2">{v.title}</h3>
                    <p className="text-[0.95rem] text-slate-600/90 dark:text-slate-300 leading-relaxed font-sans">{v.body}</p>
                  </div>
                </div>
              </FadeIn>
            ))}
        ...
        <div className="w-full max-w-[920px] mx-auto">
          <FadeIn>
            <h2 className="font-display text-[clamp(2.2rem,3.5vw,3rem)] font-black tracking-[-0.04em] leading-[1.05] mb-14 text-slate-900 dark:text-slate-100">
              Built by someone who<br /><em className="italic font-light text-indigo-600 dark:text-indigo-400">lived the problem.</em>
            </h2>
          </FadeIn>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
            {TEAM.map((m, i) => (
              <FadeIn key={m.name} delay={i * 0.07}>
                <div className="p-8 border border-slate-200/70 dark:border-slate-800 rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md h-full flex flex-col text-left transition-all duration-300 hover:shadow-md hover:border-indigo-600">
                  ...
                  <div className="font-display font-bold text-[1.15rem] text-slate-900 dark:text-slate-100 tracking-[-0.02em] mb-1">{m.name}</div>
                  <div className="inline-block text-[0.55rem] font-bold tracking-[0.22em] uppercase text-indigo-600 dark:text-indigo-400 bg-indigo-600/10 px-2 py-0.5 rounded-full w-fit mb-4">{m.role}</div>
                  <p className="text-[0.88rem] text-slate-600/90 dark:text-slate-300 leading-relaxed font-sans mt-auto">{m.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
