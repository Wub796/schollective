...
const USE_CASES = [
  { icon: <FileText className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />, title: "Research Paper Guidance", body: "Get feedback on your thesis, methodology, or literature review from a professor in exactly that field." },
  { icon: <GraduationCap className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />, title: "Graduate School Planning", body: "Ask professors about their programs, what they look for in applicants, and how to strengthen your profile." },
  { icon: <Microscope className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />, title: "Science Fair & Research Projects", body: "High schoolers: get expert perspective on your project before competition season." },
  { icon: <BookOpen className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />, title: "Understanding Complex Topics", body: "Sometimes textbooks aren't enough. Get a nuanced explanation from someone who has spent a career studying it." },
  { icon: <Users className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />, title: "Finding Research Opportunities", body: "Learn about lab openings, summer programs, and internships directly from professors actively seeking students." },
  { icon: <PenTool className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />, title: "Academic Writing Improvement", body: "Get your writing reviewed by academics who publish regularly and know exactly what clarity looks like." },
];
    ...
    <div className="bg-transparent text-slate-900 dark:text-slate-100 min-h-screen">
        ...
        <div className="w-full max-w-[760px] mx-auto flex flex-col items-center text-center">
          <FadeIn className="w-full flex flex-col items-center">
            ...
            <h1 className="font-display text-[clamp(2.8rem,6.5vw,5rem)] font-black tracking-[-0.04em] leading-[1.05] mb-8 text-slate-900 dark:text-slate-100 text-center">
              Your questions<br />
              <em className="italic font-light text-indigo-600 dark:text-indigo-400">deserve real answers.</em>
            </h1>
          </FadeIn>
        ...
        <div className="w-full max-w-[920px] mx-auto">
          <FadeIn>
            <h2 className="font-display text-[clamp(2.2rem,4vw,3.5rem)] font-black tracking-[-0.04em] leading-[1.05] mb-14 text-slate-900 dark:text-slate-100">
              From sign-up to<br /><em className="italic font-light text-indigo-600 dark:text-indigo-400">insight, in four steps.</em>
            </h2>
          </FadeIn>
            ...
            {HOW_IT_WORKS.map((s, i) => (
              <FadeIn key={s.n} delay={i * 0.06}>
                <div className="grid grid-cols-[3.5rem_1fr] gap-8 p-8 md:p-10 mb-6 border border-slate-200/70 dark:border-slate-800 rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md text-left items-start shadow-xs">
                  <span className="font-mono text-[0.68rem] font-bold tracking-[0.2em] text-white bg-indigo-600 px-2.5 py-1 rounded-full text-center mt-1">{s.n}</span>
                  <div>
                    <h3 className="font-display text-[1.35rem] font-bold text-slate-900 dark:text-slate-100 tracking-[-0.02em] mb-2">{s.title}</h3>
                    <p className="text-[0.95rem] text-slate-600/90 dark:text-slate-300 leading-relaxed font-sans">{s.body}</p>
                  </div>
                </div>
              </FadeIn>
            ))}
      ...
      <section className="relative z-10 flex flex-col items-center text-center px-8 border-t border-slate-200/60 dark:border-slate-800" style={{ paddingTop: "9rem", paddingBottom: "9rem" }}>
        <div className="w-full max-w-[920px] mx-auto">
          <FadeIn>
            <h2 className="font-display text-[clamp(2.2rem,4vw,3.5rem)] font-black tracking-[-0.04em] leading-[1.05] mb-14 text-slate-900 dark:text-slate-100">
              Every serious question<br /><em className="italic font-light text-indigo-600 dark:text-indigo-400">has a home here.</em>
            </h2>
          </FadeIn>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
            {USE_CASES.map((uc, i) => (
              <FadeIn key={uc.title} delay={i * 0.05}>
                <div className="p-8 border border-slate-200/70 dark:border-slate-800 rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md h-full transition-all duration-300 hover:shadow-md hover:border-indigo-600 text-left flex flex-col">
                  <div className="w-12 h-12 rounded-xl bg-indigo-600/10 dark:bg-indigo-500/15 border border-indigo-600/25 dark:border-indigo-500/30 flex items-center justify-center mb-6">{uc.icon}</div>
                  <h3 className="font-display text-[1.15rem] font-bold text-slate-900 dark:text-slate-100 tracking-[-0.02em] mb-2">{uc.title}</h3>
                  <p className="text-[0.88rem] text-slate-600/90 dark:text-slate-300 leading-relaxed font-sans mt-auto">{uc.body}</p>
                </div>
              </FadeIn>
            ))}
          </div>
