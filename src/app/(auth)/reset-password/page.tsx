...
function Field({ id, name, type = "text", label, placeholder, required = false }: {
  ...
  return (
    <div style={{ position: "relative" }}>
      <label htmlFor={id} style={{ display: "block", fontSize: "0.62rem", fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: focused ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.3)", marginBottom: "0.6rem", transition: "color 0.25s", fontFamily: "var(--font-sans)" }}>
        {label}
      </label>
      <input id={id} name={name} type={type} placeholder={placeholder} required={required}
        onFocus={() => setFocused(true)}
        onBlur={() => setFocused(false)}
        style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${focused ? "rgba(255,255,255,0.55)" : "rgba(255,255,255,0.12)"}`, padding: "0.75rem 0", fontSize: "1rem", color: "#fff", outline: "none", transition: "border-color 0.3s", fontFamily: "var(--font-sans)" }}
      />
      <motion.div
        animate={{ scaleX: focused ? 1 : 0, opacity: focused ? 1 : 0 }}
        transition={{ duration: 0.35, ease: EASE }}
        style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "1px", background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.8), transparent)", transformOrigin: "center" }}
      />
    </div>
  );
}
      ...
      >
        ...
        <motion.div variants={fadeUp}>
          <Link href="/" style={{ textDecoration: "none" }}>
            <span className="font-display" style={{ fontSize: "1.25rem", fontWeight: 800, color: "#fff", letterSpacing: "-0.02em" }}>Schollective</span>
          </Link>
        </motion.div>
        ...
        {/* Headline */}
        <motion.h1 variants={fadeUp} className="font-display" style={{ fontSize: "clamp(2.2rem, 5vw, 3.2rem)", fontWeight: 900, color: "#fff", letterSpacing: "-0.035em", lineHeight: 1.05 }}>
          {step === "request" ? <>Recover your<br /><em style={{ fontStyle: "italic", color: "rgba(255,255,255,0.35)" }}>access.</em></> : <>Set a new<br /><em style={{ fontStyle: "italic", color: "rgba(255,255,255,0.35)" }}>password.</em></>}
        </motion.h1>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === "request" && !sent && (
            ...
            >
              <p style={{ fontSize: "0.85rem", color: "rgba(255,255,255,0.4)", lineHeight: 1.8, fontFamily: "var(--font-sans)" }}>
              ...
              <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                style={{ width: "100%", padding: "1.1rem 2rem", background: "#fff", color: "#080c14", border: "none", borderRadius: "100px", fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.28em", textTransform: "uppercase", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.6 : 1, fontFamily: "var(--font-sans)" }}
              >
          ...
          {step === "request" && sent && (
            <motion.div key="sent" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              style={{ display: "flex", flexDirection: "column", gap: "1.5rem", padding: "2rem", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", background: "rgba(255,255,255,0.02)" }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: "rgba(120,220,120,0.8)", flexShrink: 0 }} />
                <span style={{ fontSize: "0.55rem", fontWeight: 700, letterSpacing: "0.25em", textTransform: "uppercase", color: "rgba(255,255,255,0.4)", fontFamily: "var(--font-sans)" }}>Link Sent</span>
              </div>
              <p style={{ fontSize: "0.85rem", color: "rgba(255,255,255,0.45)", lineHeight: 1.8, fontFamily: "var(--font-sans)" }}>
        ...
        <motion.div variants={fadeUp}>
          <Link href="/login" style={{ textDecoration: "none" }}>
            <span style={{ fontSize: "0.58rem", fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", fontFamily: "var(--font-sans)" }}>
              ← Back to Login
            </span>
          </Link>
        </motion.div>
